...
async function checkCollectionStatus(collectionName: string): Promise<CollectionStatus> {
  try {
    ...
    const documents = snapshot.docs.map(doc => ({
      id: doc.id,
      data: doc.data(),
      lastModified: doc.metadata.fromCache ? 'cached' : 'server'
    }));
    ...
    return {
      ...
      sampleDocuments,
      lastModified: documents.length > 0 ? 'recent' : 'never'
    };
...
async function checkDataConsistency(): Promise<ConsistencyCheck[]> {
...
async function generateSystemStatusReport(): Promise<void> {
  ...
  consistencyChecks.forEach((check: ConsistencyCheck) => {
  ...
  const totalIssues = errorCollections.length + consistencyChecks.filter((c: ConsistencyCheck) => c.status === 'FAIL').length;
  ...
  if (systemHealth !== 'HEALTHY') {
    ...
    if (consistencyChecks.some((c: ConsistencyCheck) => c.status === 'FAIL')) {
...
export { generateSystemStatusReport, checkCollectionStatus, checkDataConsistency };
